...
const ReportScreen: React.FC<ReportScreenProps> = ({ navigation }) => {
  ...
  const fetchReports = async (pageNumber = 0, isRefreshing = false) => {
    try {
      const res = await getAllReports({ page: pageNumber, size: 20 }).unwrap();
      const content: Report[] = res?.content || [];
  ...
  const handleOptionSelect = (val: string, report: Report) => {
    if (val === "View") {
    ...
    <SafeAreaView style={styles.safeContent} edges={["top", "left", "right"]}>
      ...
      <View style={styles.whiteSheet}>
        <AnimatedDatePicker
          ...
          onSelect={(date: Date) => setFromDate(formatDateForApi(date))}
        />
        ...
        <AnimatedDatePicker
          ...
          maximumDate={new Date()}
          onSelect={(date: Date) => setToDate(formatDateForApi(date))}
        />
